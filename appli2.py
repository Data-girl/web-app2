...
def run():
    ...
    if selection == 'Prédiction par lot':
        ...
        demo=st.markdown("""[1-Télécharger ce fichier de démonstration](https://drive.google.com/uc?export=download&id=1iirBBGeSma1jYT1B9JJOzgs-OMObSh7-)""")
        batch = st.file_uploader("Veuillez charger un fichier au format .csv",type=["csv"])
        if batch is not None:
            ...
            concat['predictions']=concat['predictions'].map({0:'à risque',1:'sans rique'})
            st.write(concat)
