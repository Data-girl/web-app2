...
def run():
    ...
    if selection == "Prédiction en temps réel" :
        ...
        Age = st.number_input('Age du client',min_value=1,max_value=70, value=20)
        Sexe = st.selectbox("Indiquez le sexe",["homme","femme"])
        Profession= st.selectbox("Indiquez la situation professionnelle",['qualifié', 'non_qualifié', 'haut_qualifié'])
    ...
    if selection == 'Prédiction par lot':
        ...
        st.markdown("""[1-Télécharger le fichier de démonstration du projet](https://drive.google.com/uc?export=download&id=101UcKnecN7raBzbDJQ3UQt0QrKBCIfin)""")
        ...
        if batch is not None:
            ...
            concat.columns=['Age', 'Sexe', 'Profession', 'Type_logement','Compte_epargne', 'Compte_courant', 'Credit_amount', 'Duration','predictions']
